...
const testimonials = [
  {
    ...
    avatar: "KR",
    avatarColor: "from-teal to-teal-deep",
  ...
  },
  {
    ...
    avatar: "FA",
    avatarColor: "from-indigo to-ink",
  ...
  },
  {
    ...
    avatar: "OB",
    avatarColor: "from-teal-deep to-indigo",
  ...
  },
  {
    ...
    avatar: "YK",
    avatarColor: "from-indigo-soft to-indigo",
  ...
  },
  {
    ...
    avatar: "NT",
    avatarColor: "from-ink to-teal-deep",
...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="relative py-28 md:py-32 bg-paper-deep overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] rounded-full bg-indigo/6 blur-[150px] pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full bg-teal/6 blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div ref={ref} className="text-center mb-16">
          ...
          >
            Trusted by{" "}
            <span className="italic text-brand-gradient">Web3 Leaders</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={inView ? { opacity: 1, y: 0 } : {}}
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <div className="relative rounded-3xl p-10 md:p-14 bg-paper-card border border-ink/8 overflow-hidden">
            <div className="absolute top-8 right-8 opacity-[0.06]">
              <Quote size={100} className="text-teal-deep" />
            </div>
            <div className="absolute top-0 left-0 w-60 h-60 bg-teal/6 rounded-full blur-[80px]" />
              ...
              >
                <div className="flex gap-1 mb-6">
                  {Array.from({ length: testimonials[current].stars }).map((_, i) => (
                    <Star key={i} size={16} className="text-teal-deep fill-teal-deep" />
                  ))}
                </div>

                <p className="text-ink/85 text-xl md:text-2xl leading-relaxed font-light mb-8 max-w-4xl font-display">
                  &ldquo;{testimonials[current].text}&rdquo;
                </p>

                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-full bg-gradient-to-br ${testimonials[current].avatarColor} flex items-center justify-center flex-shrink-0`}
                  >
          ...
          <div className="flex items-center justify-between mt-6">
              ...
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    i === current
                      ? "w-6 h-1.5 bg-teal-deep"
                      : "w-1.5 h-1.5 bg-ink/25 hover:bg-ink/40"
                  }`}
                />
              ))}
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="w-10 h-10 rounded-xl glass flex items-center justify-center text-ink/60 hover:text-ink transition-all"
              >
                <ChevronLeft size={18} />
              </button>
              <button
                ...
                className="w-10 h-10 rounded-xl bg-teal/10 border border-teal/25 flex items-center justify-center text-teal-deep hover:bg-teal/20 transition-all"
              >
          ...
          {testimonials.slice(0, 3).map((t, i) => (
            <motion.button
              ...
              className={`text-left rounded-2xl p-6 border cursor-pointer transition-all duration-300 ${
                current === i
                  ? "bg-paper-card border-teal/30"
                  : "bg-paper-card/60 border-ink/8 hover:border-ink/15"
              }`}
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                >
                  <span className="text-paper text-xs font-bold">{t.avatar}</span>
                </div>
                <div>
              ...
              <p className="text-ink/60 text-xs leading-relaxed line-clamp-3">{t.text}</p>
            </motion.button>
          ))}
        </div>
      </div>
